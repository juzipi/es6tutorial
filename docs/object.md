...
> 和解构赋值的功能有关系，因为在最后才能够算出还剩余的需要解构的值，如果在前面就不清楚真的的解构放到扩展里面，对象的属性是会变化。
...
z // 3
```
> 如果是const o = {x:1,y:2},是可以做到，create这是去__proto__上去找的，所以无效。
...
> 还是需要注意resetConfig中传入的参数必须有a和b，才能够真正执行baseFunction。扩展和使用相对应。
